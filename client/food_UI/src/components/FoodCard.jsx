...
function FoodCard({ id, name, restaurant, price, image, category = "Toy", rating = 4.8, onQuickView, isComparing = false, onToggleCompare, discountPrice = null }) {
  const { addToCart } = useCart();
  const { wishlist, addWishlist, removeWishlist } = useWishlist();
  const food = { _id: id, name, restaurant, price, image, category, rating, discountPrice };
  ...
  return (
    <BorderGlow
      ...
      innerClassName="relative h-full rounded-3xl"
    >
      <motion.article
        whileHover={{ y: -8 }}
      ...
      <Link to={`/foods/${id}`} className="block">
        <div className="relative overflow-hidden">
          ...
          {hasDiscount && (
            <span className="absolute left-4 top-[60px] rounded-full bg-gradient-to-r from-red-500 to-coral px-3 py-1 font-display text-xs font-bold text-white shadow-card">
              -{discountPercent}%
            </span>
          )}
        ...
        <div className="p-6 pb-3">
          ...
          <div className="mt-4 flex items-baseline gap-2">
            <p className="text-2xl font-bold text-coral">{Number(displayPrice).toLocaleString("vi-VN")}₫</p>
            {hasDiscount && (
              <p className="text-sm font-medium text-ink/40 line-through">{Number(price).toLocaleString("vi-VN")}₫</p>
            )}
          </div>
        </div>
      </Link>
