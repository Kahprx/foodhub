...
function Dashboard() {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      const [statusRes, chartRes, recentRes, topRes, lowRes, custRes, brandsRes, convRes] =
        await Promise.all([
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Hôm nay: {stats.orderToday} đơn · {fmtVND(stats.revenueToday)} · Tuần: {fmtVND(stats.revenueThisWeek)} · Tháng: {fmtVND(stats.revenueThisMonth)}
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-6">
        {stats.pendingOrders > 0 && (
          <Link
            ...
            className="flex items-center gap-3 rounded-2xl border-2 border-yellow-300 bg-yellow-50 p-4 font-semibold text-yellow-800 transition hover:bg-yellow-100"
          >
      ...
      {conversion && (
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            <div key={card.label} className="flex items-center gap-4 rounded-2xl bg-white p-5 shadow-md">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gray-50 text-2xl">{card.icon}</div>
              <div>
          ...
          <div className="space-y-4">
            ...
            {topSelling.map((f, i) => (
              <div key={f._id} className="flex items-center gap-4">
                ...
                <img src={f.image || f.images?.[0]} alt={f.name} className="h-12 w-12 rounded-xl object-cover" />
            ...
            <div className="space-y-4">
              {topBrands.map((b, i) => (
                <div key={b._id} className="flex items-center gap-4">
                  <span className="w-6 font-black text-gray-300">#{i + 1}</span>
                  <img src={b.logo || "https://ui-avatars.com/api/?name=" + encodeURIComponent(b.name || "?")} alt="" className="h-10 w-10 rounded-xl object-cover" />
                  <div className="flex-1">
        ...
        <div className="rounded-2xl bg-white p-6 shadow-md">
          ...
          <div className="space-y-3">
            ...
            {recentOrders.map((order) => (
              <Link key={order._id} to={`/admin/orders/${order._id}`} className="flex items-center justify-between rounded-xl border p-3 transition hover:bg-gray-50">
                <div>
